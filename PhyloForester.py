...
from PyQt5.QtGui import QColor, QIcon, QKeySequence, QStandardItem, QStandardItemModel
    ...
    def paint(self, painter, option, index):
        ...
        if index.column() == 1:  # Check if it's the progress bar column
            ...
            if 0 < progress_value < 100 and status == ANALYSIS_STATUS_RUNNING:
                ...
                painter.fillRect(
                    bar_x, bar_y, progress_width, bar_height, QColor(0x99CCFF)
                )  # Blue progress bar
...
class PfTabBar(QTabBar):
    ...
    def finishEditing(self):
        ...
        if new_text and new_text != self._original_text:
            ...
            self.setTabBackgroundColor(
                self._current_index, QColor("yellow")
...
class PfTableView(QTableView):
    ...
    def fill(self):
        ...
        fill_value, ok = QInputDialog.getText(
            self, "Fill Cells", "Enter value to fill selected cells:", text="0"
        )
    ...
    def setData(self, index, value, role=Qt.EditRole):
        ...
        if self.undo_stack is not None and old_value != value:
            command = EditCellCommand(
                self, index.row(), index.column(), old_value, value, old_changed, True
            )
            self.undo_stack.push(command)
        else:
...
class PhyloForesterMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.actionNewProject = QAction(
            QIcon(pu.resource_path(ICON["new_project"])), self.tr("New Project\tCtrl+N"), self
        )
        ...
        self.actionPreferences = QAction(
            QIcon(pu.resource_path(ICON["preferences"])), "Preferences", self
        )
    ...
    def read_settings(self):
        ...
        if self.m_app.remember_geometry is True:
            ...
            self.setGeometry(
                self.m_app.settings.value("WindowGeometry/MainWindow", QRect(100, 100, 1400, 800))
            )
        else:
        ...
        if clicked_button == btnYes:
            ...
            if pu.create_result_directory(default_path):
                ...
                QMessageBox.information(
                    self, "Success", f"Result directory created successfully:\n\n{default_path}"
                )
            ...
            if custom_path:
                ...
                if pu.create_result_directory(custom_path):
                    ...
                    QMessageBox.information(
                        self, "Success", f"Result directory set to:\n\n{custom_path}"
                    )
    ...
    def startAnalysis(self, analysis=None):
        ...
        if not analysis:
            try:
                analysis_list = (
                    PfAnalysis.select()
                    .where(PfAnalysis.analysis_status == ANALYSIS_STATUS_READY)
                ...
                QMessageBox.critical(self, "Database Error", f"Failed to access database:\n{e}")
                return
            except Exception as e:
        ...
        try:
            ...
            QMessageBox.critical(self, "File Error", f"Failed to save analysis data:\n{e}")
        ...
        if self.analysis.analysis_type == ANALYSIS_TYPE_PARSIMONY:
            ...
            command_filename = self.create_mrbayes_command_file(
                data_filename, self.analysis.result_directory, self.analysis
            )
    ...
    def create_mrbayes_command_file(self, data_filename, result_directory, analysis):
        ...
        command_text = f"""begin mrbayes;
   set autoclose=yes nowarn=yes;
   execute {data_filename};
   lset nst={analysis.mcmc_nst} rates={analysis.mcmc_nrates};
   mcmc nruns={analysis.mcmc_nruns} ngen={analysis.mcmc_ngen} samplefreq={analysis.mcmc_samplefreq} file={data_filename} burnin={analysis.mcmc_burnin} Savebrlens=No;
   sump burnin={analysis.mcmc_burnin};
   sumt burnin={analysis.mcmc_burnin} Showtreeprobs=No;
end;"""
        # print(command_text)
    ...
    def generate_consensus_tree(self, analysis):
        ...
        if analysis.analysis_type == ANALYSIS_TYPE_ML:
            ...
            tree_filename = os.path.join(
                analysis.result_directory, analysis.datamatrix.datamatrix_name + ".phy.treefile"
            )
    ...
    def update_analysis_info(self, analysis):
        ...
        if tree_item is not None and not sip.isdeleted(tree_item):
            ...
            analysis_status = {
                ...
                "percentage": analysis.completion_percentage,
            }
        ...
        if isinstance(dm, PfDatamatrix):
            ...
            if ret == 1:
                ...
                if (
                    ...
                    and self.selected_datamatrix.id in self.data_storage["datamatrix"]
                ):
                    self.hsplitter.replaceWidget(
                        1, self.data_storage["datamatrix"][self.selected_datamatrix.id]["widget"]
                    )
                # self.reset_tableView()
    ...
    def treeView_drop_event(self, event):
        ...
        if event.source() == self.treeView:
            ...
            if source_object is not None and isinstance(source_object, PfAnalysis):
                return
            if source_object is not None and isinstance(source_object, PfDatamatrix):
    ...
    def add_datamatrix(self, file_name_list):
        ...
        if self.selected_project is None:
            ...
            msgBox.setText(
                "No project is selected. Do you want to create a new project with this file?"
            )
    ...
    def show_selected_analysis(self):
        ...
        if self.selected_analysis.id in self.data_storage["analysis"]:
            ...
            if av is None:
                av = self.data_storage["analysis"][self.selected_analysis.id]["widget"] = (
                    AnalysisViewer(logger=self.logger)
                )
    ...
    def update_datamatrix_table(self):
        ...
        if self.selected_datamatrix.id in self.data_storage["datamatrix"]:
            self.data_storage["datamatrix"][self.selected_datamatrix.id]["widget"] = (
                self.create_datamatrix_table(self.selected_datamatrix)
            )
    ...
    def on_treeview_selection_changed(self, selected, deselected):
        ...
        if indexes:
            ...
            if isinstance(data, PfAnalysis):
                ...
                self.hsplitter.replaceWidget(
                    1, self.data_storage["analysis"][self.selected_analysis.id]["widget"]
                )
            ...
            if isinstance(data, PfDatamatrix):
                ...
                if len(self.data_storage["project"][data.id]["datamatrices"]) > 0:
                    ...
                    self.hsplitter.replaceWidget(
                        1, self.data_storage["datamatrix"][dm_id]["widget"]
                    )
                else:
                    self.hsplitter.replaceWidget(1, self.empty_widget)
    ...
    def load_treeview(self):
        ...
        for project in project_list:
            ...
            self.data_storage["project"][project.id] = {
                "object": project,
                "item": item1,
                "datamatrices": [],
            }
            ...
            if project.datamatrices.count() > 0:
                ...
                for dm in dm_list:
                    ...
                    self.data_storage["datamatrix"][dm.id] = {
                        "object": dm,
                        "item": item3,
                        "analyses": [],
                        "widget": None,
                    ...
                    if dm.analyses.count() > 0:
                        for analysis in dm.analyses:
                            ...
                            if analysis.id not in self.data_storage["analysis"]:
                                self.data_storage["analysis"][analysis.id] = {
                                    "object": analysis,
                                    "tree_item": item6,
                                    "widget": None,
                                }
    ...
    def load_datamatrices(self, project=None):
        ...
        self.datamatrix_list = (
            project.datamatrices
        )  # PfDatamatrix.select().where(PfDatamatrix.project == self.selected_project)
...
if __name__ == "__main__":
    ...
    app.settings = QSettings(
        QSettings.IniFormat, QSettings.UserScope, pu.COMPANY_NAME, pu.PROGRAM_NAME
    )

    translator = QTranslator()
    app.language = app.settings.value("language", "en")
    translator.load(pu.resource_path(f"translations/PhyloForester_{app.language}.qm"))
