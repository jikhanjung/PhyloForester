    ...
    def update_info(self, analysis: PfAnalysis) -> None:
        ...
        if analysis.completion_percentage == 100:
            ...
            if os.path.isfile(log_filename):
                ...
                log_fd = open(log_filename, encoding="utf-8")
        ...
        if analysis.finish_datetime is not None:
            self.edtAnalysisFinishDatetime.setText(
                analysis.finish_datetime.strftime("%Y-%m-%d %H:%M:%S")
            )
...
class TreeViewer(QWidget):
    def __init__(self, logger: Optional[logging.Logger] = None) -> None:
        ...
        self.combo_tree_style.currentIndexChanged.connect(
            self.on_combo_tree_style_currentIndexChanged
        )
        ...
        self.combo_font_size.currentIndexChanged.connect(
            self.on_combo_font_size_currentIndexChanged
        )
    ...
    def on_cbx_all_characters_clicked(self):
        if self.cbx_all_characters.isChecked():
            for i in range(self.character_model.rowCount()):
                ...
                self.character_model.setData(
                    self.character_model.index(i, 0), Qt.Checked, Qt.CheckStateRole
                )
        else:
            for i in range(self.character_model.rowCount()):
                # set all characters to unchecked
                self.character_model.setData(
                    self.character_model.index(i, 0), Qt.Unchecked, Qt.CheckStateRole
                )
    ...
    def on_btn_export_clicked(self):
        ...
        if self.combo_image_type.currentText() == "SVG":
            filename, _ = QFileDialog.getSaveFileName(
                self, "Save File", "", "Scalable Vector Graphics (*.svg)"
            )
            if filename:
    ...
    def on_btn_bookmark_clicked(self):
        ...
        if self.tree_type == 1:
            ...
            if self.analysis.analysis_type == ANALYSIS_TYPE_PARSIMONY:
                ...
                tree_to_save.tree_name = (
                    TREE_TYPE_BOOTSTRAP + " " + str(self.tree_current_index + 1)
                )
            elif self.analysis.analysis_type == ANALYSIS_TYPE_BAYESIAN:
                tree_to_save.tree_type = TREE_TYPE_POSTERIOR
                tree_to_save.tree_name = (
                    TREE_TYPE_POSTERIOR + " " + str(self.tree_current_index + 1)
                )
            ...
            self.bookmarked_newick_tree_list = [
                tree.newick_text for tree in self.bookmarked_tree_list
            ]
            # self.bookmarked_newick_tree_list = []
        ...
        if self.tree_type == 1:
            if self.tree_current_index not in self.treeobj_hash.keys():
                if self.tree_current_index < len(self.newick_tree_list):
                    tree = Phylo.read(
                        io.StringIO(self.newick_tree_list[self.tree_current_index - 1]), "newick"
                    )
            ...
            if self.tree_current_index < len(self.bookmarked_newick_tree_list):
                ...
                tree = Phylo.read(
                    io.StringIO(self.bookmarked_newick_tree_list[self.tree_current_index]), "newick"
                )
    ...
    def set_tree_image_buf(self, buf: Any) -> None:
        ...
        self.tree_label.curr_pixmap = self.tree_label.orig_pixmap.scaled(
            int(
                self.tree_label.orig_pixmap.width()
                * self.tree_label.scale
                / self.tree_label.image_canvas_ratio
            ),
            int(
                self.tree_label.orig_pixmap.height()
                * self.tree_label.scale
                / self.tree_label.image_canvas_ratio
            ),
        )
    ...
    def set_analysis(self, analysis: PfAnalysis) -> None:
        ...
        if self.analysis.analysis_type == ANALYSIS_TYPE_PARSIMONY:
            ...
            self.analysis.analysis_type == ANALYSIS_TYPE_ML
            or self.analysis.analysis_type == ANALYSIS_TYPE_BAYESIAN
        ):
            # self.cbx_align_taxa.setEnabled(False)
    ...
    def load_trees(self):
        ...
        if self.analysis.analysis_type == ANALYSIS_TYPE_ML:
            ...
            filename = os.path.join(
                tree_dir,
                self.analysis.datamatrix.datamatrix_name.replace(" ", "_") + ".phy.boottrees",
            )
            ...
            tf.readtree(
                os.path.join(
                    tree_dir, self.analysis.datamatrix.datamatrix_name.replace(" ", "_") + ".nex.t"
                ),
                "Nexus",
            )
            self.newick_tree_list = tf.tree_list
...
class TreeLabel(QLabel):
    ...
    def _2canx(self, coord):
        return (
            round((float(coord) / self.image_canvas_ratio) * self.scale)
            + self.pan_x
            + self.temp_pan_x
        )
    ...
    def draw_tree(self, painter):
        ...
        """ calculate depths """
        if (
            self.tree_style == pu.TREE_STYLE_BRANCH_LENGTH
            and self.analysis_type != ANALYSIS_TYPE_PARSIMONY
        ):
            ...
            for clade in [c for c in self.tree.find_clades(order="postorder")]:
                ...
                for child in clade:
                    if (
                        self.clade_depths[child] - self.clade_depths[clade]
                        < self.node_minimum_offset
                    ):
                        self.clade_depths[clade] = (
                            self.clade_depths[child] - self.node_minimum_offset
                        )
        ...
        if self.tree_style == pu.TREE_STYLE_TOPOLOGY and self.char_mapping:
            character_states_list = [
                clade.character_states for clade in clade_list if clade.name is not None
            ]
        # get font
        font = painter.font()
        ...
        for idx, taxon in enumerate(taxa_list):
            ...
            if self.tree_style == pu.TREE_STYLE_TOPOLOGY and self.char_mapping:
                taxon_text = (
                    "".join([character_states_list[idx][i] for i in self.character_index_list])
                    + " "
                    + taxon
                )
        ...
        if self.tree_style == pu.TREE_STYLE_TIMETREE:
            ...
            for i in range(len(taxa_list)):
                self.taxa_timetable[taxa_list[i]] = [
                    float(self.timetable[i][0]),
                    float(self.timetable[i][1]),
                ]
        ...
        self.branch_length_scale = (self.tree_image_width * 0.9 - max_text_width) / (
            self.max_depth - self.min_clade_depth
        )
    ...
    def map_characters_to_nodes(self):
        # return
        pu.reconstruct_ancestral_states(
            self.tree,
            self.analysis.datamatrix.datamatrix_as_list(),
            self.analysis.datamatrix.get_taxa_list(),
        )
    ...
    def draw_node(self, painter, node, begin_row, depth):
        ...
        if node.is_terminal():
            ...
            if self.tree_style == pu.TREE_STYLE_BRANCH_LENGTH:
                ...
                depth = self.clade_depths[node]
                text = (
                    "".join([str(node.character_states[x]) for x in self.character_index_list])
                    + " "
                    + text
                )
        ...
        for child in node:
            ...
            if (
                self.tree_style == pu.TREE_STYLE_BRANCH_LENGTH
                or self.tree_style == pu.TREE_STYLE_TOPOLOGY
                and self.char_mapping
            ):
                ...
                if child.is_terminal():
                    terminal_from_depth = (
                        self.max_fad - self.taxa_timetable[child.name][0] - self.min_clade_depth
                    )
                    terminal_to_depth = (
                        self.max_fad - self.taxa_timetable[child.name][1] - self.min_clade_depth
                    )
            ...
            if self.tree_style == pu.TREE_STYLE_TOPOLOGY and self.char_mapping:
                ...
                self.draw_transition_text(
                    painter, (from_depth + to_depth) / 2, v_pos, transition_list
                )
    ...
    def paintEvent(self, event):
        ...
        painter.fillRect(
            self.rect(), QBrush(QColor(self.bgcolor))
        )  # as_qt_color(COLOR['BACKGROUND'])))
        ...
        image_rect = QRect(
            self.pan_x + self.temp_pan_x,
            self.pan_y + self.temp_pan_y,
            self.tree_image_width,
            self.tree_image_height,
        )
...
class AnalysisDialog(QDialog):
    ...
    def select_result_directory(self):
        result_directory = str(
            QFileDialog.getExistingDirectory(
                self, "Select a folder", str(self.edtResultDirectory.text())
            )
        )
        if result_directory:
    ...
    def Run(self):
        ...
        if self.cbxParsimony.isChecked():
            ...
            if result_directory_base.find(" ") != -1:
                QMessageBox.warning(
                    self, "Result Directory", "Result directory should not contain space."
                )
                return
        ...
        for analysis_type in analysis_type_list:
            ...
            if analysis_type == ANALYSIS_TYPE_PARSIMONY:
                ...
                directory_name = (
                    analysis.datamatrix.datamatrix_name
                    + " "
                ...
                )  # TNT does not like space in file name
                ...
                # Normalize path for OS-appropriate separators
                analysis.result_directory = os.path.normpath(
                    os.path.join(result_directory_base, directory_name)
                )
    ...
    def read_settings(self):
        self.remember_geometry = pu.value_to_bool(
            self.m_app.settings.value("WindowGeometry/RememberGeometry", True)
        )
        if self.remember_geometry is True:
            self.setGeometry(
                self.m_app.settings.value(
                    "WindowGeometry/AnalysisDialog", QRect(100, 100, 600, 400)
                )
            )
        else:
        ...
        self.result_directory_base = os.path.normpath(
            self.m_app.settings.value("ResultPath", pu.DEFAULT_RESULT_DIRECTORY)
        )
...
class DatamatrixDialog(QDialog):
    ...
    def read_settings(self):
        ...
        if self.remember_geometry is True:
            self.setGeometry(
                self.m_app.settings.value(
                    "WindowGeometry/DatamatrixDialog", QRect(100, 100, 600, 400)
                )
            )
        else:
            self.setGeometry(QRect(100, 100, 600, 400))
    ...
    def set_datamatrix(self, datamatrix):
        ...
        if len(character_list) != 0:
            for idx, character in enumerate(character_list):
                ...
                item.setData(
                    Qt.UserRole,
                    {"original_name": character, "original_index": idx, "is_new": False},
                )
                self.lstCharacters.addItem(item)
        ...
        if len(taxa_list) != 0:
            for idx, taxon in enumerate(taxa_list):
                ...
                # Store metadata: original name, index, and whether it's new
                item.setData(
                    Qt.UserRole, {"original_name": taxon, "original_index": idx, "is_new": False}
                )
                self.lstTaxa.addItem(item)
    ...
    def Delete(self):
        ret = QMessageBox.question(
            self,
            "",
            "Are you sure to delete this matamatrix?",
...
class ProjectDialog(QDialog):
    ...
    def read_settings(self):
        ...
        if self.remember_geometry is True:
            self.setGeometry(
                self.m_app.settings.value("WindowGeometry/ProjectDialog", QRect(100, 100, 600, 400))
            )
        else:
            self.setGeometry(QRect(100, 100, 600, 400))
            self.move(self.parent.pos() + QPoint(100, 100))
...
class PreferencesDialog(QDialog):
    ...
    def select_result_path(self):
        result_path = str(
            QFileDialog.getExistingDirectory(self, "Select a folder", str(self.m_app.result_path))
        )
        if result_path:
    ...
    def select_iqtree_path(self):
        iqtree_path = str(
            QFileDialog.getOpenFileName(self, "Select IQTree", str(self.m_app.iqtree_path))[0]
        )
        if iqtree_path:
    ...
    def select_mrbayes_path(self):
        mrbayes_path = str(
            QFileDialog.getOpenFileName(self, "Select Mr.Bayes", str(self.m_app.mrbayes_path))[0]
        )
        if mrbayes_path:
    ...
    def read_settings(self):
        self.m_app.remember_geometry = pu.value_to_bool(
            self.m_app.settings.value("WindowGeometry/RememberGeometry", True)
        )
        ...
        self.m_app.result_path = (
            os.path.normpath(result_value) if result_value else pu.DEFAULT_RESULT_DIRECTORY
        )
        ...
        if self.m_app.remember_geometry is True:
            self.setGeometry(
                self.m_app.settings.value(
                    "WindowGeometry/PreferencesDialog", QRect(100, 100, 600, 400)
                )
            )
        else:
            self.setGeometry(QRect(100, 100, 600, 400))
    ...
    def write_settings(self):
        ...
        self.m_app.settings.setValue(
            "WindowGeometry/RememberGeometry", self.m_app.remember_geometry
        )
        ...
        self.m_app.settings.setValue(
            "SoftwarePath/TNT", os.path.normpath(self.m_app.tnt_path) if self.m_app.tnt_path else ""
        )
        self.m_app.settings.setValue(
            "SoftwarePath/IQTree",
        ...
        self.m_app.settings.setValue(
            "SoftwarePath/MrBayes",
            os.path.normpath(self.m_app.mrbayes_path) if self.m_app.mrbayes_path else "",
        )
        self.m_app.settings.setValue(
            "ResultPath", os.path.normpath(self.m_app.result_path) if self.m_app.result_path else ""
        )
    ...
    def select_folder(self):
        folder = str(
            QFileDialog.getExistingDirectory(self, "Select a folder", str(self.data_folder))
        )
        if folder:
